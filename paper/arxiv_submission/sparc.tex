% ============================================================
%  arXiv preprint version — acmart sigconf (authors visible)
%  (For the IROS submission, use [sigconf,anonymous,review])
% ============================================================
...
\acmConference[IROS '26]{IEEE/RSJ International Conference on
  Intelligent Robots and Systems (IROS)}{October 2026}{}
\acmYear{2026}
\copyrightyear{2026}
\acmISBN{}
\acmDOI{}
...
\title{SPARC: Spatial-Aware Path Planning via Attentive Agent
  Communication}

% ── Authors (auto-anonymized by the `anonymous' class option) ─
\author{Sayang Mu}
\authornote{These authors contributed equally to this work.}
\affiliation{%
  \institution{Nanyang Technological University}
  \country{Singapore}}
\email{sayang.mu@ntu.edu.sg}
...
% ============================================================
\begin{abstract}
Efficient communication is critical for decentralized
Multi-Agent Path Finding (MAPF), yet existing learned
communication methods treat all neighboring agents equally
regardless of their spatial proximity, leading to diluted
attention in congested regions where coordination matters most.
We propose Relation-enhanced Multi-Head Attention (\rmha), a
communication mechanism that explicitly embeds pairwise Manhattan
distances into the attention weight computation, enabling each
agent to dynamically prioritize messages from spatially relevant
neighbors.
Combined with a distance-constrained attention mask and GRU-gated
message fusion, \rmha{} integrates seamlessly with MAPPO for
stable end-to-end training.
In zero-shot generalization from 8 training agents to 128 test
agents on $40\!\times\!40$ grids, \rmha{} achieves approximately
75\% success rate at 30\% obstacle density---outperforming the
best baseline by over 25 percentage points.
Ablation studies confirm that distance-relation encoding is the
key contributor to success rate improvement in high-density
environments.
\end{abstract}

\keywords{Multi-agent path finding, graph attention mechanism,
  multi-head attention, communication optimization, cooperative
  decision-making}
...
Multi-Agent Path Finding (MAPF) is a core problem in artificial
intelligence and robotics, aiming to design collision-free paths
for multiple agents to navigate from their respective starting
positions to goal positions in a known
environment~\cite{wang2023scrimp}.
In the standard MAPF formulation, a graph structure represents the
agents' workspace together with sets of start and goal
vertices.
At each time step, an agent may either stay at its current vertex
or move to an adjacent one~\cite{li2021lifelong}.
A solution to a MAPF instance is a set of paths—one per agent—that
are free of both vertex and edge collisions.
Finding such a solution is known to be NP-hard.

A MAPF instance is defined by a graph, a set of agents, and their
...
given instance.
A MAPF \emph{algorithm} (or \emph{solver}) refers to the method
used to compute such a solution.

MAPF has broad applications in intelligent warehousing
...
As a foundational task in building cooperative multi-agent
systems, MAPF plays a pivotal role in the design phase and its
importance is expected to grow as demand for cooperative and
competitive multi-agent scenarios increases.

%\begin{figure}[t]
%\centering
%\includegraphics[width=\columnwidth]{image}
%\caption{Application scenarios of MAPF.}
...
increasingly deployed in unmanned warehouses, where each AMR
acts as an agent in the underlying scheduling problem.
For such AMR-based warehousing systems, path planning is the core
scheduling component.
However, as the number of agents and the complexity of tasks
...
Extensions to multi-agent settings include LRA*, which adjusts
...
level computes optimal single-agent paths under
...
agent initially follows its individual optimal path; upon
collision, only the conflicting agents are coupled for joint
...
scalability as the number of agents and environment size grow.
...
In MAPF, the problem is commonly modeled as a
...
with MARL;
Li~\etal~first introduced GNNs for MAPF, encoding FOV
interactions and supporting decentralized
decisions~\cite{li2020gnn,li2021magat}.

\subsection{Motivation and Challenges}

Despite progress in learned communication for MAPF, two key
challenges remain.
\textbf{(C1) Spatially-agnostic communication:}
Existing methods such as MAGAT~\cite{li2021magat} and
SCRIMP~\cite{wang2023scrimp} compute attention weights purely from
message content, treating a distant agent identically to an
adjacent one. In congested regions, this dilutes attention across
irrelevant neighbors and degrades coordination.
\textbf{(C2) Training instability at scale:}
Transformer-based communication introduces high-variance gradients
in online MARL, causing training collapse when naively applied to
large agent populations.
Our work addresses both challenges by embedding spatial relations
directly into the attention mechanism and constraining
communication to local neighborhoods.
...
\item We propose a \textbf{spatial relation-enhanced multi-head
      ...
      inter-agent relative distance as edge information into the
...
Communication for Multi-Agent Path Finding}

\subsection{System Model and Problem Formulation}

\subsubsection{MAPF System Model}

The MAPF problem can be formally defined as a four-tuple
$\langle \mathcal{A}, G, \mathcal{O}_{\mathrm{dyn}},
\mathcal{O}_{\mathrm{static}} \rangle$, where:
$\mathcal{A}=\{A_1, A_2, \ldots, A_N\}$ is the set of $N$ agents;
...
Each agent $A_n \in \mathcal{A}$ is represented by a tuple
$\langle s_n, g_n \rangle$, where $s_n$ and $g_n$ are the start
and goal positions, respectively.
A valid MAPF solution must satisfy the constraints illustrated
in Fig.~\ref{fig:mrpp_constraints}:
...
For any two agents $A_n$ and $A_{n'}$ and any time step
$0 \le t \le T$, their positions must not coincide, nor may any
agent occupy a position held by a static or dynamic obstacle.
\item \textbf{Edge collision-free constraint:}
No two agents may simultaneously traverse the same edge in
opposite directions.
\end{enumerate}

When multiple valid solutions exist, MAPF can be cast as an
...
\end{equation}
where $|\Pi_k|$ denotes the path length of agent $k$.
...
\Description{Three diagrams illustrating MAPF collision types
that must be avoided: two agents occupying the same vertex
(vertex collision), two agents traversing the same edge in
opposite directions (edge collision), and two adjacent agents
swapping positions (swap collision).}
\caption{Illustration of MAPF constraints: vertex collisions,
edge collisions, and swap collisions among agents must all be
avoided.}
\label{fig:mrpp_constraints}
\end{figure}

\textbf{Reward design in RL-based MAPF.}
Table~\ref{tab:reward_survey} summarizes the reward mechanisms
used in representative RL-based MAPF methods for agents.
...
\begin{table*}[t]
\centering
\caption{Comparison of Reward Designs in RL-based MAPF Methods.}
...
In this work, the MAPF problem is defined by a set of agents
...
Time is discretized into steps; at each step an agent may move to
...
\rangle$, where
$\mathcal{N}=\{1,\ldots,N\}$ is the agent set;
...
At each time step $t$, agent~$i$ receives observation
...
\subsubsection{MDP Modeling for MAPF}

As illustrated in Fig.~\ref{fig:marl_modeling}, MAPF can be
modeled as a Markov Game (MG), an extension of the Markov Decision
Process (MDP) to multi-agent settings~\cite{huang2021strategic}.
An MDP is defined by a five-tuple $\langle S, A, T, R, \gamma
\rangle$.
In an MG with $N$ agents, the action space extends to the joint
...
(POMG)~\cite{ma2021distributed}, where each agent observes only
...
\Description{Schematic comparing three multi-agent RL modeling
paradigms for MAPF: a Markov Game with full observability, a
Partially Observable Markov Game where each agent sees a local
view, and a Dec-POMDP that adds a conditional observation
distribution.}
\caption{MARL modeling paradigms for MAPF: MG, POMG, and
Dec-POMDP.}
\label{fig:marl_modeling}
\end{figure}

To cast MAPF as an RL problem, we construct a discrete
two-dimensional grid world.
Agents, goals, and obstacles each occupy one cell.
At the start of each episode, $n$ unique start and goal positions
are randomly assigned within the same connected region.
Agents move simultaneously and collisions are checked on the joint
action vector.
Each agent's FOV is limited to $3\times 3$.

\textbf{State space $\mathcal{S}$:}
Each agent's observation consists of two parts.
...
that action brings the agent closer to its goal) and four maps
encoding nearby obstacles, other agents' positions, the agent's
...
Agents that have not yet reached their goals receive a per-step
penalty to encourage faster completion.
An episode terminates when all agents reach their goals or a
...
The architecture of the proposed \rmha{} module is shown in
Fig.~\ref{fig:rmha_arch}.
To design an efficient communication paradigm for MARL-based
MAPF, we adopt a communication
...
inter-agent spatial relationships into the attention
computation~\cite{xiao2025collaborative,chen2025predicting,chen2023transportation}.
Specifically, the Manhattan distance between agents is embedded
...
distance between the corresponding agents.
We choose Manhattan distance over Euclidean distance because
agents move on a grid with four cardinal directions, making
Manhattan distance the exact shortest-path metric in obstacle-free
regions and a tighter lower bound in the presence of obstacles.
The encoded observations $(o_1,\ldots,o_N)$ thus capture not only
individual agent information but also higher-level spatial
relationships through communication.

\textbf{Communication range constraint.}
In practice, each agent can only communicate with a limited number
of peers due to bandwidth and medium-access contention.
To model this, each agent communicates only with those within a
specified radius $R$:
\begin{equation}\label{eq:comm_neighbor}
  \mathcal{N}_i(t) = \{j \mid d_{ij}(t) \le R,\; j \ne i\},
\end{equation}
where $d_{ij}(t)$ is the distance between agents $i$ and $j$ at
time $t$.
...
\begin{equation}\label{eq:mask}
  ...
  \end{cases}
\end{equation}
ensuring that only information from connected agents is accessible.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{rmha-module}
\Description{Block diagram of the RMHA communication module:
local observations are combined with Manhattan-distance
embeddings, fed through query and key projections, masked by
the communication radius, and aggregated via multi-head
attention to produce updated messages.}
...
For agent $i$ at time $t$, the local observation
...
computation.
For a group of $n$ agents, the queries, keys, and values are:
\begin{align}
...
(iv)~a blocking flag indicating whether the agent obstructs
other agents' paths.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{rmha-network}
\Description{Full network architecture of RMHA combined with
MAPPO, showing the convolutional observation encoder, LSTM
temporal fusion, distance-aware multi-head attention
communication block, and the four output heads producing
policy, value, message vector, and a blocking flag.}
...
\Description{Pseudocode of the RMHA training procedure: it
initializes encoder, Transformer, and output heads, then loops
over episodes and time steps to encode observations, exchange
distance-aware messages across attention layers, generate
policy and value outputs, resolve conflicts, compute rewards,
and update parameters via PPO.}
\hrule\smallskip
\noindent\textbf{Algorithm~\thealgctr}\quad \rmha{} Training Procedure
\label{alg:rmha}\smallskip\hrule\smallskip
{\small
\noindent\textbf{Input:} Agent count $N$, episodes $K$, steps $T$,
FOV $F$, layers $L$, discount $\gamma$, PPO clip $\epsilon$\\[2pt]
1: Initialize encoder $\phi$, Transformer $\psi$, output heads $\theta$, buffer $\mathcal{B}$\\
2: \textbf{for} episode $k = 0, \ldots, K\!-\!1$ \textbf{do}\\
3: \quad\textbf{for} step $t = 0, \ldots, T\!-\!1$ \textbf{do}\\
4: \quad\quad Encode observations via $\phi$ (Eqs.~\ref{eq:conv}--\ref{eq:lstm})\\
5: \quad\quad Compute messages and distances for all agents\\
6: \quad\quad \textbf{for} $l = 1, \ldots, L$: attention with distance (Eqs.~\ref{eq:Q}--\ref{eq:msg}), GRU, FFN\\
7: \quad\quad Generate $\pi_i^t, v_i^t, b_i^t$ from Transformer output + LSTM state\\
8: \quad\quad Resolve conflicts via softmax over value differences\\
9: \quad\quad Compute intrinsic rewards; execute actions; store in $\mathcal{B}$\\
10: \quad\textbf{end for}\\
11: \quad Sample mini-batch; compute GAE; update $\phi,\psi,\theta$ via PPO (Eq.~\ref{eq:ppo})\\
12: \textbf{end for}
}\smallskip\hrule
\end{figure}
...
\textbf{Training.}
The number of agents is set to 8 with a $3\times 3$ FOV.
...
A representative training environment is shown in
Fig.~\ref{fig:grid_env}; full parameter settings are listed in
Table~\ref{tab:params}.

\textbf{Testing.}
The agent count is scaled to 128 on a $40\!\times\!40$ grid at
...
\Description{Example 10-by-10 grid world used for training, with
30 percent obstacle density and 8 agents. Colored squares mark
agent start positions, colored circles mark their goals, gray
circles mark static obstacles, and a red dashed box outlines
the 3-by-3 field of view.}
...
To further verify generalization across different agent counts,
the model trained with 8 agents is tested with 16, 32, 64, and
128 agents.
Fig.~\ref{fig:scalability} shows that while success rate decreases
as the number of agents increases, \rmha{} maintains high success
rates from 16 to 64 agents and avoids performance collapse at 128
agents, demonstrating good scalability.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{scalability}
\Description{Line plot showing the success rate of RMHA as the
number of agents increases from 16 to 128, with the model
trained only at 8 agents. Success rate decays gradually but
avoids collapse at the largest scale.}
\caption{Success rate of \rmha{} under different agent counts.}
...
\textbf{Baselines.}
We compare with three state-of-the-art MARL-based MAPF methods:
...
\textbf{Metrics.}
\begin{itemize}
\item \textbf{Max Reached (MR):} the maximum number of agents
  that reach their goals at any time step.
\item \textbf{Collision Rate (CO):} the ratio of collision events
  (with obstacles or other agents) to the product of episode
  length and agent count.
\item \textbf{Success Rate (SR):} the percentage of episodes in
  which all agents reach their goals collision-free within the
  time limit $T_{\max}$:
...
\item \textbf{MAPPO}: the communication module is entirely
  removed; agents never send or receive messages.
\item \textbf{MAPPO+Graph Comm}: attention weights are computed
  based on message content alone, without fusing Manhattan
  distance embeddings.
\end{itemize}
All experiments use 128 agents in a $40\!\times\!40$ grid at
obstacle densities of 0\%, 15\%, and 30\%.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{ablation-reward}
\Description{Training reward curves over time for three
ablation variants: MAPPO without communication, MAPPO with
graph communication, and RMHA. The two communication-enabled
variants converge faster and more stably than MAPPO alone.}
...
\begin{figure}[t]
\centering
\subfloat[MAPPO vs.\ Graph Comm]{\includegraphics[width=0.48\columnwidth]{ablation-sr-mappo-vs-graphcomm}\label{fig:ablation_sr1}}
\hfill
\subfloat[\rmha{} vs.\ Graph Comm]{\includegraphics[width=0.48\columnwidth]{ablation-sr-rmha-vs-graphcomm}\label{fig:ablation_sr2}}
\Description{Two bar charts comparing success rate at 0, 15,
and 30 percent obstacle density. Left: MAPPO without
communication versus graph communication; graph communication
wins at every density. Right: graph communication versus RMHA;
RMHA wins by a larger margin as density increases.}
\caption{Communication ablation: success rate comparison.}
\label{fig:ablation_sr}
\end{figure}

As shown in Fig.~\ref{fig:ablation_sr}, success rates decrease
with increasing obstacle density.
MAPPO+Graph Comm consistently outperforms MAPPO---at 30\% density,
MAPPO drops to approximately 40\% while Graph Comm remains around
50\%.
\rmha{} further improves upon Graph Comm, achieving approximately
75\% at 30\% density (vs.\ 50\%), confirming that Manhattan
distance-based relation encoding enables more accurate spatial
understanding.
In terms of maximum goals reached, \rmha{} maintains near 125
goals even in high-density environments, while MAPPO degrades
rapidly.

Overall, in the 128-agent, 30\%-density setting, the success rate
...
All experiments use a $40\!\times\!40$ grid with 128 agents.
...
\Description{Grouped bar chart comparing success rate of
ODrM*, SCRIMP, DHC, PICO, and RMHA on Warehouse and City/Game
maps at 0, 15, and 30 percent obstacle complexity. RMHA
maintains the highest success rate as complexity grows.}
...
\Description{Grouped bar chart of maximum goals reached
(out of 128) for each baseline and RMHA across Warehouse and
City/Game maps. SCRIMP and RMHA hold above 120 even at 30
percent density while PICO and DHC drop sharply.}
...
mechanisms, as they lack effective modeling of dynamic inter-agent
relationships.
...
more precise modeling of local congestion structure.
...
clustering in local areas, with some agents stalling for extended
periods.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{trajectory-comparison}
\Description{Side-by-side overhead view of agent trajectories
on the same grid. The RMHA panel on the left shows
well-organized, non-overlapping paths; the MAPPO panel on the
right shows tangled paths and stalled agents.}
...
\Description{Four-step storyboard of two agents A and B meeting
head-on in a narrow corridor. The baseline leaves them
deadlocked at steps T2 and T3, while under RMHA agent A yields
sideways at T3 so agent B passes, and both reach their goals
by T4.}
\caption{Deadlock resolution in a narrow corridor.
At $T_1$, agents A and B enter a conflict;
the baseline leads to deadlock by $T_2$--$T_3$.
Under \rmha, agent~A yields at $T_3$, allowing B to pass, and
both proceed by $T_4$.}
\label{fig:deadlock}
\end{figure}
...
Under the baseline, both agents adopt similar forward/wait
strategies, resulting in persistent deadlock.
Under \rmha, the attention mechanism enables agent~A to perceive
the conflicting neighbor's proximity and proactively yield,
...
multi-agent path finding from the perspectives of communication
...
By incorporating relative distance information between agents into
...
inter-agent distance information in optimizing path planning
decisions.

In summary, \rmha{} provides an effective solution for efficient
cooperation in complex multi-agent environments, offering a new
perspective on intelligent agent communication and collaboration.

\textbf{Limitations and future work.}
The current evaluation is limited to discrete grid worlds with
uniform agent capabilities; extending \rmha{} to continuous action
spaces and heterogeneous agent teams remains open.
Additionally, the Manhattan distance embedding assumes obstacle-free
line-of-sight proximity, which may not reflect true reachability in
cluttered environments.
Future work will explore learned distance metrics that account for
obstacle topology, as well as integration with lifelong MAPF
settings where start--goal assignments change dynamically.
