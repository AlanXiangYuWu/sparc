...
**image 的 8通道含义**：
```
通道1-4: 启发式地图 — 往上/下/左/右走是否更靠近目标（0/1二值）
...
```
image (N,8,3,3) ──→ CNN(4层卷积+MaxPool) ──→ 64维
                                               │
vector (N,7) ───────────────────────────→ 拼接(71维)
                                               │
                                      FC [71→256→256→256]
                                     ...
                                     encoded_features(N,256)
                                               │
distance_matrix(N,N) ─→ RMHA通信模块 ──→ updated_messages(N,256)
                                               │
                           拼接 [encoded + messages] (N,512)
                                               │
                     ┌─────────────────────────┤
                     │               │                  │
                  Policy网络      Value网络         Blocking预测
                动作分布(5类)  外在+内在价值估计      是否阻塞他人
```
...
| 5. Blocking Predictor | 41,217 | 预测是否阻碍他人路径 |
| **Total** | **1,072,160** | **约107万参数** |
...
*本笔记综合整理自：项目源码分析、论文正文（参考文章辅导稿件修改稿全稿完成的.md）、RMHA_vs_标准MHA对比.md、ProjectNotebook.md（Q&A学习记录）。用于面试准备。*
